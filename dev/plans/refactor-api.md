...
- Support Pydantic v2 only. Use a guarded module-scope Pydantic import that
  caches optional symbols and treats candidates as non-Pydantic when it is
  unavailable. Declare Pydantic in a
...
`env`, `pos`, `opt`, and `secret`. `short` is an optional string; an empty
string or `None` means there is no short option. Defaults remain ordinary
parameter defaults,
...
4. Add a dependency-free import check that runs without optional extras, such
   ...
   not installed. Use a guarded module-scope `try`/`except ImportError` import
   so cached Pydantic symbols remain optional.
...
2. Define `CliArg(help="", short="", env=None, pos=False, opt=True,
   secret=False)`. `short` accepts a string or `None`; do not normalize it into
   a collection. Do not add a
...
3. In `msup/base.py`, replace the dataclass-only assumptions around
   ...
   internal field record containing name, unwrapped annotation, every generic
   `Annotated` metadata value, default, and whether a default factory exists.
   Keep it CLI-agnostic and internal unless callers require it.
4. Read annotations with `get_type_hints(..., include_extras=True)`. Base
   conversion strips generic `Annotated` wrappers while retaining their
   metadata; CLI construction interprets only `CliArg` values from that shared
   record.
...
- Complete: defined immutable `CliArg` and its metadata unwrapping in
  `msup.cli`; `short` accepts an empty string or `None` when no short option
  is wanted.
- Complete: removed `cliarg(...)`, migrated the in-repository test fixtures,
  and retained the public `CliArg` declaration in `msup.cli`.
- Complete: made `msup.base` CLI-agnostic. Its shared field record preserves
  generic `Annotated` metadata without importing or inspecting `CliArg`.
...
1. Add Pydantic v2 detection in `msup/base.py` using a guarded `try`/`except
   ImportError` import at module scope that caches optional Pydantic symbols.
...
3. Define a shared `is_structured_model` check for dataclasses and Pydantic v2
   ...
   with `Model.model_validate(values)` so Pydantic performs its own validation.
4. Update `to_dict`, `to_kwargs`, and `from_dict` to obtain unwrapped field
   annotations from common discovery. Delegate public Pydantic conversion to
   `model_dump()` and `model_validate()` so Pydantic owns its native aliases
...
- Verify Pydantic remains optional by exercising dataclass conversion without
  Pydantic installed; do not simulate an unavailable installed package by
  mutating `sys.modules`.

### Success criteria

- Pydantic v2 models round trip through all public serialization helpers.
- Installing `msup` without Pydantic remains valid.
...
- Complete: added public `is_pydantic_model` detection in `msup.base` with a
  guarded module-scope optional import; base and CLI imports remain valid
  without Pydantic and Pydantic v1 compatibility classes raise an explicit
...
- Complete: public Pydantic conversion delegates directly to `model_dump()`
  and `model_validate()`, preserving native aliases and validation semantics.
  Missing Pydantic inputs remain omitted so the model controls defaults.
...
### Review follow-up: Pydantic field and native conversion contract

**Status:** complete

- Extended the shared internal `InitArg` record to retain the unwrapped base
  annotation and all `Annotated` metadata values. Structured CLI parsing now
  reads `CliArg` from this record instead of resolving model hints again.
- Made `is_pydantic_model` public, cached optional Pydantic imports at module
  scope, and retained explicit Pydantic v1 rejection.
- Changed public Pydantic conversion to use `model_dump()` and
  `model_validate()` directly. Focused tests cover metadata preservation,
  class and instance detection, v1 rejection, and alias-aware native model
  validation.

### Review follow-up: Preserve dataclass nested defaults

**Status:** complete

- Generalize nested structured-field omission so a dataclass field with a
  declared default or default factory is omitted when no config, environment,
  CLI, or dotted child source supplies it. Keep constructing a required
  dataclass child without a default, and retain the existing Pydantic
  omission behavior.
- Rename `has_nested_default_value` to `has_nested_source` because it detects
  supplied nested input rather than declared defaults.
- Add a regression test whose nested dataclass default factory differs from
  the child class default, proving that an empty invocation preserves the
  parent factory result.

Completed by omitting an absent nested field when it has no nested source and
the parent is Pydantic or the dataclass field declares a default. The renamed
`has_nested_source` now states its source-detection contract directly, and the
new regression fixture confirms that `ChildArgs(count=2)` remains the parent
default-factory result instead of being replaced by `ChildArgs()`.
...
### Review follow-up: Simplify CLI metadata helpers

**Status:** complete

- Keep `CliArg.short` optional with an empty-string default; both empty and
  `None` mean that only the long option is registered.
- Reuse `InitArg` for direct function parameters so model and function
  metadata share one field shape. Rename the newly introduced CLI helpers
  without leading underscores where they are used across parser stages, and
  remove the unused structured-first-argument helper.
- Rename the cached optional Pydantic model symbols without leading
  underscores. Rename the nested Pydantic default-preservation predicate to
  `has_nested_default_value`, while retaining its source-detection behavior.
- Remove the redundant structured-model check from the Pydantic nested-default
  preservation branch after its enclosing function establishes that invariant.
- Remove redundant docstrings from the small CLI metadata type and helpers.
- Extend CLI tests for an empty short name and a direct-parameter short option.

Completed by keeping `CliArg.short` typed as `str | None` with an empty-string
default, treating both falsey forms as long-option-only metadata. Direct
parameters now reuse `InitArg`, and the cross-stage CLI/Pydantic helpers use
public names while parser-local helpers remain private. The shared
`unwrap_annotated` helper provides one generic metadata-unwrapping path for
both base conversion and CLI signature discovery. The redundant nested-model
guard and new self-explanatory docstrings were removed.
