...
class InitArg:
    def __init__(self, name: str, default: Any = MISSING, type: Any = None):
        self.name = name
...
def load_callable(name: str):
    parts = name.split(".")
    for maybe_idx in range(len(parts) - 1, 0, -1):
        try:
            result = importlib.import_module(".".join(parts[:maybe_idx]))
        except ModuleNotFoundError:
            continue
        for part in parts[maybe_idx:]:
...
def _from_value(x: T, field_type: type, concrete_type: type, field_name: str):
    if x is None:
        ...
        elif annotation_origin(field_type) in (Union, UnionType):
...
def _to_dict_value(x: T, field_type: type):
    ...
    if annotation_origin(field_type) in (Union, UnionType):
...
def to_dict(x: T) -> dict:
    hints = _dataclass_hints(type(x)) if is_dataclass(x) else {}
    result = {}
    for f in fields_or_init_kwargs(type(x)):
        if hasattr(x, f.name):
            field_type = hints.get(f.name, f.type)
            value = getattr(x, f.name)
            result[f.name] = _to_dict_value(value, field_type or type(value))
    return result
...
def from_dict(clazz: type, x: dict) -> T:
    hints = _dataclass_hints(clazz) if is_dataclass(clazz) else {}
    construct_args = {}
    for f in fields_or_init_kwargs(clazz):
        field_type = hints.get(f.name, f.type)
        if f.name in x:
            construct_args[f.name] = _from_value(x[f.name], field_type or type(x[f.name]), type(x[f.name]), f.name)
